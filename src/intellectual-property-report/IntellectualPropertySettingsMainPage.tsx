...
export function IntellectualPropertySettingsMainPage() {
    ...
    const [updateTimeRecordMutation, updateTimeRecordMutationResult] =
        useMutation<UpdateTimeRecordMutation>(UpdateTimeRecordCategory);
    const [deleteTimeRecordMutation, deleteTimeRecordMutationResult] =
        useMutation<DeleteTimeRecordMutation>(DeleteTimeRecordCategory);
    const [createTimeRecordMutation, createTimeRecordMutationResult] =
        useMutation<CreateTimeRecordMutation>(CreateTimeRecordCategory);
        ...
        return (
            <Stack direction={'column'} sx={{width: '400px'}}>
                <SimpleCrudList
                    ...
                    list={[...(data.allTimeRecordCategories as TimeRecordCategory[])]
                        .sort(
                            ComparatorBuilder.comparing<TimeRecordCategory>(
                                timeRecordCategory => timeRecordCategory.name
                            )
                                .thenComparing(timeRecordCategory => timeRecordCategory.id)
                                .build()
                        )
