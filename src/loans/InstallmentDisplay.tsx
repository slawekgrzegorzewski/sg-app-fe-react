...
export function InstallmentDisplay({installment, onClick = noOp}: InstallmentDisplayProps) {
    function repaidInstallment() {
        if (installment.repaidInterest.amount > 0) {
            return (
                <>
                    <StandOutText>
                        <CurrencyAmountDisplay {...installment.repaidInterest} />
                    </StandOutText>{' '}
                    odsetek,{' '}
                </>
            );
    ...
    function repaidAmount() {
        if (installment.repaidAmount.amount > 0) {
            return (
                <>
                    <StandOutText>
                        <CurrencyAmountDisplay {...installment.repaidAmount} />
                    </StandOutText>{' '}
                    kapitału,{' '}
                </>
            );
        ...
        if (installment.overpayment.amount > 0) {
            return (
                <>
                    nadpłacono{' '}
                    <StandOutText>
                        <CurrencyAmountDisplay {...installment.overpayment} />
                    </StandOutText>
                    ,{' '}
                </>
            );
    ...
    return (
        ...
        >
            Spłacona dnia <StandOutText>{installment.paidAt}</StandOutText>,{repaidInstallment()}
            {repaidAmount()}
            {overpayment()}
        </Box>
    );
