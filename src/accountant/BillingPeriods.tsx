...
export function BillingPeriods() {
    ...
    if (loading) {
        ...
        const billingPeriod = billingPeriodResponse?.billingPeriod
            ...
            : null;
        const billingPeriodCreationBlocker = billingPeriodResponse?.creationBlockers
            ? billingPeriodResponse.creationBlockers
            : null;
        const incomesByCategory =
            billingPeriod?.incomes.reduce((map, income) => {
        ...
        const expensesByCategory =
            billingPeriod?.expenses.reduce((map, expense) => {
        ...
        return (
            <Stack direction="column" alignItems="center" sx={{px: {xs: 1, sm: 2}, py: 2}}>
                <Stack direction="column" sx={{width: '100%', maxWidth: 800}}>
                    ...
                    {!data.billingPeriod.billingPeriod &&
                        billingPeriodCreationBlocker &&
                        noCreationBlockers(billingPeriodCreationBlocker) && (
                                ...
                                <Button
                                    ...
                                    onClick={async () => {
                                        await client
                                            ...
                                            .then(() =>
                                                createBillingPeriodMutation({
                                                    variables: {yearMonth: dayjs(yearMonth).format(YEAR_MONTH_FORMAT)},
                                                })
                                            )
                                            .then(() => refetch())
                                            .then(() => Promise.resolve());
                        ...
                        !noCreationBlockers(billingPeriodCreationBlocker) && (
                            <Stack direction="column" sx={{mt: 2, px: 1.5, py: 1}}>
                                <Typography variant="h5" sx={{mb: 1, color: 'secondary.main'}}>
                                    Nie można utworzyć tego okresu rozliczeniowego
                                </Typography>
                                <Typography variant="body1" color="text.secondary">
                                    Możliwe przyczyny:
                                </Typography>
                                <Typography variant="body1" sx={{pl: 1.5}}>
                                    a) poprzedni okres rozliczeniowy nie został zakończony
                                </Typography>
                                <Typography variant="body1" sx={{pl: 1.5}}>
                                    b) przeglądasz miesiąc inny niż bieżący
                                </Typography>
                            </Stack>
                        )}
