...
export function LoadingIndicator({label = 'Ładowanie...'}: LoadingIndicatorProps) {
    return <Stack direction={'row'} spacing={2} alignItems={'center'} sx={{padding: 2}}
...
export function ErrorDisplay({error, title = 'Nie udało się pobrać danych', onRetry}: ErrorDisplayProps) {
    return <Alert severity={'error'} sx={{margin: 2}}
                  action={onRetry
                      ? <Button color={'inherit'} size={'small'} onClick={onRetry}>Ponów</Button>
                      : undefined}>
